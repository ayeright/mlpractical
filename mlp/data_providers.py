...
class MNISTDataProvider(DataProvider):
    ...
    def to_one_of_k(self, int_targets):
        ...
        """
        N, K = len(int_targets), int_targets.max() + 1
        one_hot = np.zeros((N, K))
        one_hot[range(N), int_targets] = 1
        return one_hot
...
class MetOfficeDataProvider(DataProvider):
    ...
    def __init__(self, window_size, batch_size=10, max_num_batches=-1,
        ...
        data = np.loadtxt(data_path, skiprows=3)[:, 2:]
        ...
        i = np.tile(np.arange(window_size), (n_sequences, 1)).astype(int)
        offsets = np.tile(np.ones(window_size), (n_sequences, 1)).astype(int) * np.arange(n_sequences)[:, None]
        i = i + offsets
        sequences = x[i]
